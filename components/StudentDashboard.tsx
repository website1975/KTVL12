...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user }) => {
  ...
  const refreshData = async () => {
    ...
    // Cập nhật thông tin user mới nhất (có points)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-10">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
            ...
            <div className="text-right">
                ...
                <span className="bg-blue-600 text-white px-5 py-2 rounded-full text-xs font-black shadow-lg shadow-blue-100 uppercase">LỚP {user.grade}</span>
            </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm flex items-center gap-5">
      ...
      {/* THÔNG BÁO QUY ĐỊNH ĐIỂM THƯỞNG */}
      <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-125 transition-transform duration-700">
              <Sparkles size={120}/>
          </div>
          <div className="flex flex-col md:flex-row md:items-center gap-8 relative z-10">
              <div className="shrink-0 bg-blue-600 w-20 h-20 rounded-[2rem] flex items-center justify-center shadow-2xl shadow-blue-500/20"><Medal size={40}/></div>
              <div className="flex-1">
                  <h3 className="text-xl font-black uppercase tracking-tight mb-2">Chương trình rèn luyện tích điểm</h3>
                  <p className="text-slate-400 text-xs font-medium leading-relaxed max-w-2xl italic">Cố gắng rèn luyện để tích lũy điểm thưởng. Tổng điểm tích lũy sẽ được quy đổi thành điểm cộng thực tế vào cuối học kỳ.</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white/5 border border-white/10 p-4 rounded-2xl">
                      <p className="text-blue-400 text-[9px] font-black uppercase tracking-widest mb-1">Kiểm tra (Test)</p>
                      <p className="text-white text-xs font-bold">Điểm ≥ 8.0: <span className="text-emerald-400">+1đ</span></p>
                  </div>
                  <div className="bg-white/5 border border-white/10 p-4 rounded-2xl">
                      <p className="text-purple-400 text-[9px] font-black uppercase tracking-widest mb-1">Luyện tập (Practice)</p>
                      <p className="text-white text-xs font-bold">Thời gian ≥ 45p: <span className="text-emerald-400">+1đ</span></p>
                  </div>
              </div>
          </div>
      </div>
                ...
                <div key={q.id} className="bg-white rounded-[2.5rem] border border-slate-200 p-8 flex flex-col hover:shadow-2xl hover:-translate-y-2 transition-all group relative overflow-hidden border-b-8 border-b-slate-50 hover:border-b-blue-600">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-auto">
                    ...
                    <button 
                        ...
                        className="flex items-center justify-center gap-2 bg-blue-600 text-white py-3.5 rounded-2xl text-[10px] font-black uppercase shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all"
                    >
          ...
          <div className="fixed inset-0 bg-slate-900/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
              <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up border-8 border-white">
                <div className="p-8 bg-slate-900 text-white flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mt-1">Chế độ xem trước • {previewQuiz.questions.length} câu hỏi • {previewQuiz.durationMinutes} phút</p>
                        </div>
                    </div>
                            ...
                            <div key={q.id} className="bg-white p-10 rounded-[2rem] shadow-sm border border-slate-100">
                                ...
                                {q.type === 'mcq' && q.options && (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ml-12">
                                        {q.options.map((opt, oi) => <div key={oi} className="text-sm font-medium text-slate-500"><span className="text-slate-300 mr-2 font-black">{String.fromCharCode(65+oi)}.</span> <LatexText text={opt}/></div>)}
                                    </div>
                                )}
                ...
                <div className="p-8 bg-white border-t flex justify-center shadow-2xl relative z-10">
                    <button onClick={() => { setActiveQuiz(previewQuiz); setPreviewQuiz(null); }} className="px-16 py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs shadow-2xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all">Bắt đầu làm bài thi ngay</button>
                </div>
              </div>
          </div>
      )}
