        ...
        <div className="flex h-screen bg-[#f8fafc] overflow-hidden text-slate-700 font-sans">
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
                    ...
                    {activeMenu === 'results' && <ResultsBoard results={results} quizzes={quizzes} chapters={chapters} rGradeFilter={rGradeFilter} setRGradeFilter={setRGradeFilter} rChapterFilter={rChapterFilter} setRChapterFilter={setRChapterFilter} rQuizFilter={rQuizFilter} setRQuizFilter={setRQuizFilter} onClearCache={handleClearLocalCache} onViewHistory={(s, q, h) => setHistoryModal({studentName: s, quizTitle: q, history: h})} onDeleteResult={async (h) => {if(confirm('Xóa?')) { await Promise.all(h.map(x => deleteResult(x.id))); refreshData(); }}} />}
                    {activeMenu === 'ai' && <AIRenderer grade={grade} setGrade={setGrade} isLoading={isAiLoading} onGenerate={async (p, p1, p2, p3) => { setIsAiLoading(true); const qs = await generateQuizFromPrompt({grade, topic: p, part1Count: p1, part2Count: p2, part3Count: p3}); setQuestions(qs); setTitle(`Đề AI: ${p}`); setActiveMenu('editor'); setIsAiLoading(false); }} />}
                            ...
                            <div className="flex-1 overflow-y-auto p-10 bg-slate-50 space-y-8">
                                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                                    ...
                                    <table className="w-full text-left">
                                        <thead><tr className="bg-white border-b text-[8px] font-black uppercase text-slate-300 tracking-[0.2em]"><th className="p-6">Tên đề thi</th><th className="p-6 text-center">Điểm số</th><th className="p-6 text-center">Ngày nộp</th></tr></thead>
                                        <tbody className="divide-y">
                                          ...
                                          {results.filter(r => r.studentCode === selectedStudent.studentCode).length === 0 && (
                                              <tr><td colSpan={3} className="p-10 text-center text-xs text-slate-300 italic">Chưa có lịch sử làm bài</td></tr>
                                          )}
                                        </tbody>
                                    </table>
                                </div>
