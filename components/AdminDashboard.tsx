...
const AdminDashboard = () => {
  ...
  const getBankQuestions = (type: QuestionType) => {
      // Lấy tất cả câu hỏi từ các đề cùng Khối và cùng Chương
      const bank: Question[] = [];
      quizzes
                        ...
                        <div key={q.id} className="bg-white p-8 rounded-[2.5rem] border shadow-sm relative group animate-fade-in-up">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                                <div className="space-y-4">
                                    ...
                                    <textarea className="w-full p-6 bg-slate-50 border border-slate-100 rounded-3xl text-sm font-bold outline-none focus:ring-4 focus:ring-blue-50 transition-all min-h-[120px]" value={q.text} onChange={e => { const n = [...questions]; const i = n.findIndex(x => x.id === q.id); n[i].text = e.target.value; setQuestions(n); }} placeholder="Nhập câu hỏi (hỗ trợ LaTeX $...$)" />
                                </div>
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Xem trước (Preview)</label>
                                    ...
                                    {q.subQuestions?.map((sq, si) => (
                                        <div key={si} className="flex flex-col md:flex-row md:items-center gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                                            ...
                                            <div className="flex bg-white rounded-xl p-1.5 border">
                                                {['True', 'False'].map(v => (
                                                    <button key={v} onClick={() => { const n = [...questions]; const i = n.findIndex(x => x.id === q.id); n[i].subQuestions![si].correctAnswer = v as any; setQuestions(n); }} className={`px-5 py-2 text-[10px] font-black rounded-lg transition-all ${sq.correctAnswer === v ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}>
                                                        {v === 'True' ? 'ĐÚNG' : 'SAI'}
                                                    </button>
                                                ))}
                                            </div>
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0 z-20 shadow-2xl">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {[
            ...
            // Fix: Removed reference to undefined setSelectedStudent
              ...
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm space-y-8">
                <div className="flex items-center justify-between border-b pb-6">
                    ...
                    <div className="flex bg-slate-50 p-1.5 rounded-2xl border">
                        {['practice', 'test'].map(t => (
                            <button key={t} onClick={() => setQuizType(t as any)} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${quizType === t ? 'bg-white text-blue-600 shadow-md border' : 'text-slate-400 hover:text-slate-600'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredQuizzes.map(q => (
                    <div key={q.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 hover:shadow-2xl transition-all group relative flex flex-col">
                      ...
                      <button onClick={() => setPreviewQuiz(q)} className="mt-auto pt-6 border-t flex items-center justify-center gap-2 text-[10px] font-black text-blue-600 uppercase tracking-widest hover:underline group-hover:translate-x-1 transition-transform">
                  ...
                  <div className="p-8 bg-slate-900 text-white flex justify-between items-center">
                      <div className="flex items-center gap-4">
                          ...
                          <div>
                              ...
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic">Lọc theo: Khối {grade} • Chương {category || 'Tất cả'}</p>
                          </div>
                      </div>
                            ...
                            <div key={q.id} className="bg-white p-10 rounded-[2rem] shadow-sm border border-slate-100 relative">
                                <div className="text-slate-800 text-[15px] font-bold mb-6 leading-relaxed flex items-start gap-4">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        {q.imageUrl && <img src={q.imageUrl} className="max-w-full rounded-2xl border" />}
                                    </div>
                                </div>
                                {/* MCQ options review */}
