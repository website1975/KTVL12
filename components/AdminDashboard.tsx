                            ...
                            <div className="shrink-0">
                                {q.imageUrl ? (
                                    <div className="relative group/img">
                                        <img src={q.imageUrl} className="w-24 h-24 object-cover rounded-2xl border" alt="question content" />
                                        <button 
                                            onClick={() => {
                                                const newList = [...questions];
                                                const i = newList.findIndex(x => x.id === q.id);
                                                newList[i].imageUrl = undefined;
                                                setQuestions(newList);
                                            }} 
                                    ...
                                    <div key={oi} className="flex items-center gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                                        ...
                                        <input 
                                            ...
                                            onChange={e => {
                                                const newList = [...questions];
                                                const i = newList.findIndex(x => x.id === q.id);
                                                newList[i].options![oi] = e.target.value;
                                                setQuestions(newList);
                                            }} 
                                    ...
                                    <div key={si} className="flex flex-col md:flex-row md:items-center gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100">
                                        ...
                                        <input 
                                            ...
                                            onChange={e => {
                                                const newList = [...questions];
                                                const i = newList.findIndex(x => x.id === q.id);
                                                newList[i].subQuestions![si].text = e.target.value;
                                                setQuestions(newList);
                                            }} 
                                            ...
                                            {['True', 'False'].map(v => (
                                                <button 
                                                    ...
                                                    onClick={() => {
                                                        const newList = [...questions];
                                                        const i = newList.findIndex(x => x.id === q.id);
                                                        newList[i].subQuestions![si].correctAnswer = v as any;
                                                        setQuestions(newList);
                                                    }} 
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                <textarea 
                                    ...
                                    onChange={e => {
                                        const newList = [...questions];
                                        const i = newList.findIndex(x => x.id === q.id);
                                        newList[i].solution = e.target.value;
                                        setQuestions(newList);
                                    }} 
          ...
          {[
            ...
            <button 
                ...
                onClick={() => { 
                    ...
                    if(m.id === 'create') { setEditingId(null); setTitle(''); setQuestions([]); } 
                }} 
            ...
            <div className="max-w-5xl mx-auto space-y-12 pb-32 animate-fade-in">
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm space-y-8">
                <div className="flex items-center justify-between border-b pb-6">
                    <input 
                        type="text" 
                        className="text-3xl font-black outline-none bg-transparent placeholder-slate-200 w-full" 
                    ...
                    <div className="flex bg-slate-50 p-1.5 rounded-2xl border">
                        {['practice', 'test'].map(t => (
                            <button 
                                ...
                                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${quizType === t ? 'bg-white text-blue-600 shadow-md border' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                {t === 'practice' ? 'Luyện tập' : 'Kiểm tra'}
                            </button>
                        ))}
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-slate-300 uppercase px-1">Khối lớp</label>
                        <select className="w-full border rounded-2xl p-4 text-xs font-black bg-slate-50 uppercase outline-none" value={grade} onChange={e => { setGrade(e.target.value as Grade); setCategory(''); }}>
                            <option value="10">Lớp 10</option><option value="11">Lớp 11</option><option value="12">Lớp 12</option>
                        </select>
                    </div>
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-slate-300 uppercase px-1">Chương học</label>
                        <select className="w-full border rounded-2xl p-4 text-xs font-black bg-slate-50 uppercase outline-none" value={category} onChange={e => setCategory(e.target.value)}>
                            <option value="">Chọn chương học</option>
                            {chapters.filter(c => c.grade === grade).map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                        </select>
                    </div>
                    <div className="space-y-2">
                        <label className="text-[9px] font-black text-slate-300 uppercase px-1">Thời gian (Phút)</label>
                        <input type="number" className="w-full border rounded-2xl p-4 text-xs font-black bg-slate-50 outline-none" value={duration} onChange={e => setDuration(parseInt(e.target.value))} placeholder="Phút" />
                    </div>
                    {quizType === 'test' && (
                        <div className="space-y-2">
                            <label className="text-[9px] font-black text-slate-300 uppercase px-1">Thời gian bắt đầu</label>
                            <input type="datetime-local" className="w-full border rounded-2xl p-4 text-xs font-black bg-slate-50 outline-none" value={startTime} onChange={e => setStartTime(e.target.value)} />
                        </div>
                    )}
                </div>
                <button onClick={handleSave} className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black uppercase text-xs flex items-center justify-center gap-3 hover:bg-black transition-all shadow-2xl shadow-slate-200">
                    <Save size={20}/> Lưu Đề Thi Vào Hệ Thống
                </button>
              </div>

              <QuestionSection 
                title="PHẦN I. Câu trắc nghiệm nhiều phương án lựa chọn" 
                type="mcq" icon={CheckCircle2} 
                questions={questions} setQuestions={setQuestions}
                onAdd={handleAddQuestion} onOpenBank={(t) => setShowBank({ type: t, open: true })}
                onUploadImage={handleImageUpload} uploadingId={uploadingId}
              />
              <QuestionSection 
                title="PHẦN II. Câu trắc nghiệm đúng sai" 
                type="group-tf" icon={HelpCircle} 
                questions={questions} setQuestions={setQuestions}
                onAdd={handleAddQuestion} onOpenBank={(t) => setShowBank({ type: t, open: true })}
                onUploadImage={handleImageUpload} uploadingId={uploadingId}
              />
          ...
          {activeMenu === 'quizzes' && (
            <div className="space-y-8 animate-fade-in">
              <div className="flex flex-col lg:flex-row gap-4 items-center bg-white p-5 rounded-[2rem] border shadow-sm">
                ...
                <div className="flex gap-2">
                    <select className="px-4 py-3 bg-white border rounded-2xl text-[10px] font-black uppercase" value={quizGradeFilter} onChange={e => { setQuizGradeFilter(e.target.value as any); setQuizChapterFilter('all'); }}>
                    ...
                    <select className="px-4 py-3 bg-white border rounded-2xl text-[10px] font-black uppercase min-w-[180px]" value={quizChapterFilter} onChange={e => setQuizChapterFilter(e.target.value)}>
                        <option value="all">TẤT CẢ CHƯƠNG</option>
                        {chapters.filter(c => quizGradeFilter === 'all' || c.grade === quizGradeFilter).map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                    </select>
                </div>
              </div>
                    ...
                    return (
                        <div key={q.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-[0_20px_50px_rgba(168,85,247,0.15)] hover:border-purple-200 transition-all duration-500 group relative flex flex-col">
                          <div className="flex justify-between items-start mb-8">
                            <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${q.type === 'practice' ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'}`}>
                                {q.type === 'practice' ? 'LUYỆN TẬP' : 'KIỂM TRA'}
                            </span>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                              <button onClick={() => { 
                                  setEditingId(q.id); setTitle(q.title); setGrade(q.grade); 
                                  setDuration(q.durationMinutes); setQuestions(q.questions); 
                                  setCategory(q.category || ''); setQuizType(q.type); 
                                  setStartTime(q.startTime || ''); setActiveMenu('create'); 
                              }} className="p-2.5 bg-slate-50 text-slate-400 hover:text-blue-600 rounded-xl"><Edit size={16}/></button>
                          ...
                          <div className="bg-slate-50/70 rounded-3xl p-6 grid grid-cols-2 gap-4 mb-8 border border-transparent group-hover:bg-purple-50 group-hover:border-purple-100 transition-all duration-500">
                            <div className="text-center">
                                <div className="flex items-center justify-center gap-1.5 mb-1">
                                    ...
                                    <p className="text-[9px] font-black text-slate-400 group-hover:text-purple-500 uppercase">Lượt làm</p>
                                </div>
                            ...
                            <div className="text-center border-l border-slate-200 group-hover:border-purple-200">
                                <div className="flex items-center justify-center gap-1.5 mb-1">
                                    ...
                                    <p className="text-[9px] font-black text-slate-400 group-hover:text-purple-500 uppercase">Điểm cao</p>
                                </div>
                                <p className="text-sm font-black text-purple-600">{stats.max.toFixed(1)}</p>
                            </div>
                          </div>
                ...
                <div className="bg-white p-10 rounded-[3rem] border shadow-sm space-y-6">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex gap-3">
                            <input type="text" className="flex-1 p-5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold outline-none" placeholder="Tên chương học..." id="ch-name" />
                            ...
                            }} className="bg-blue-600 text-white px-10 rounded-2xl font-black text-xs uppercase shadow-xl hover:bg-blue-700 transition-all">Lưu Chương</button>
                        </div>
                    </div>
                </div>
                        ...
                        <div key={g} className="space-y-3">
                            ...
                            {chapters.filter(c => c.grade === g).map(c => (
                                <div key={c.id} className="bg-white p-6 px-10 rounded-[2rem] border border-slate-200 flex justify-between items-center group hover:border-blue-400 transition-all shadow-sm">
                                    ...
                                    <button onClick={async () => { if(confirm('Xóa chương này?')) { await deleteChapter(c.id); refreshData(); } }} className="p-3 text-slate-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                                        <Trash2 size={20}/>
                                    </button>
                                </div>
                            ))}
          ...
          <div className="fixed inset-0 bg-slate-900/90 z-[1100] flex items-center justify-center p-4 backdrop-blur-md">
              <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-up">
                  <div className="p-8 bg-slate-900 text-white flex justify-between items-center shrink-0">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center"><Database size={24}/></div>
                        <div><h3 className="text-lg font-black uppercase tracking-tight">Ngân hàng câu hỏi {showBank.type.toUpperCase()}</h3></div>
                      </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-10 bg-slate-50 space-y-6">
                      ...
                      {getBankQuestions(showBank.type).length === 0 && <div className="text-center py-20 text-slate-400 font-bold italic">Chưa có câu hỏi nào trong ngân hàng phù hợp.</div>}
                  </div>
              </div>
          </div>
      )}
