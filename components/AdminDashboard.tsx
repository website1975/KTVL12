...
import { 
    ...
    HelpCircle, AlignLeft, BookOpen, Calendar, Eye, XCircle, Target
} from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-700">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 bg-white px-6 py-4 rounded-2xl border shadow-sm flex items-center gap-4">
                  ...
                  <input type="text" className="flex-1 outline-none text-xs font-bold bg-transparent" placeholder="Tìm kiếm đề thi..." value={quizSearch} onChange={e => setQuizSearch(e.target.value)} />
                </div>
                <div className="flex gap-2">
                  <select className="bg-white px-4 py-2 rounded-2xl border shadow-sm text-[10px] font-black uppercase outline-none" value={quizGradeFilter} onChange={e => setQuizGradeFilter(e.target.value as any)}>
                    <option value="all">Tất cả Khối</option>
                    <option value="10">Khối 10</option>
                    <option value="11">Khối 11</option>
                    <option value="12">Khối 12</option>
                  </select>
                  <select className="bg-white px-4 py-2 rounded-2xl border shadow-sm text-[10px] font-black uppercase outline-none max-w-[200px]" value={quizChapterFilter} onChange={e => setQuizChapterFilter(e.target.value)}>
                ...
                {filteredQuizzes.map(q => {
                  ...
                  return (
                    <div key={q.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 hover:shadow-2xl transition-all group relative flex flex-col">
                      ...
                      <div className="bg-slate-50/50 rounded-3xl p-6 grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-fade-in">
              <div className="bg-white p-10 rounded-[3rem] border shadow-sm space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleAddQuestion('mcq')} className="flex-1 bg-blue-600 text-white rounded-2xl text-[9px] font-black uppercase flex items-center justify-center gap-1 shadow-lg shadow-blue-100 hover:bg-blue-700"><CheckCircle2 size={14}/> MCQ</button>
                    <button onClick={() => handleAddQuestion('group-tf')} className="flex-1 bg-purple-600 text-white rounded-2xl text-[9px] font-black uppercase flex items-center justify-center gap-1 shadow-lg shadow-purple-100 hover:bg-purple-700"><HelpCircle size={14}/> Đ/S</button>
                    <button onClick={() => handleAddQuestion('short')} className="flex-1 bg-orange-500 text-white rounded-2xl text-[9px] font-black uppercase flex items-center justify-center gap-1 shadow-lg shadow-orange-100 hover:bg-orange-600"><AlignLeft size={14}/> NGẮN</button>
                  </div>
                </div>
                <button onClick={handleSave} className="w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black uppercase text-xs flex items-center justify-center gap-3 hover:bg-black transition-all shadow-2xl shadow-slate-200"><Save size={20}/> Lưu Đề Thi</button>
              </div>
                  ...
                  <div key={q.id} className="bg-white p-10 rounded-[3rem] border shadow-sm relative group animate-fade-in-up">
                    ...
                    {q.type === 'short' && (
                        <div className="flex items-center gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                            ...
                            <input type="text" className="flex-1 bg-transparent text-lg font-black text-blue-600 outline-none" value={q.correctAnswer} onChange={e => { const n = [...questions]; n[idx].correctAnswer = e.target.value; setQuestions(n); }} placeholder="Nhập kết quả chính xác..." />
                        </div>
                    )}
                  </div>
                ))}
          ...
          {/* VIEW: QUẢN LÝ HỌC SINH (GIỮ NGUYÊN) */}
          {activeMenu === 'students' && (
                  ...
                  <div className="space-y-8 animate-fade-in">
                      ...
                      <div className="bg-white rounded-[3rem] border shadow-sm overflow-hidden">
                          ...
                          <table className="w-full text-left">
                              <thead>
                                  <tr className="border-b bg-slate-50/20"><th className="p-8 text-[11px] font-black text-slate-400 uppercase">Tên đề thi</th><th className="p-8 text-[11px] font-black text-slate-400 uppercase text-center">Điểm</th><th className="p-8 text-[11px] font-black text-slate-400 uppercase text-right">Thời gian nộp</th></tr>
                              </thead>
          ...
          {/* VIEW: BẢNG ĐIỂM TỔNG (GIỮ NGUYÊN) */}
          {activeMenu === 'results' && (
          ...
          <div className="fixed inset-0 bg-slate-900/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up border-8 border-white">
                <div className="p-8 bg-slate-900 text-white flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-5">
                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center"><BookOpen size={24}/></div>
                        <div>
                            <h3 className="text-lg font-black uppercase leading-tight">{previewQuiz.title}</h3>
                            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Chế độ xem thử • {previewQuiz.questions.length} câu hỏi • Lớp {previewQuiz.grade}</p>
                        </div>
                    </div>
                        ...
                        {previewQuiz.questions.map((q, i) => (
                            <div key={q.id} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                                ...
                                <div className="mt-8 pt-6 border-t border-slate-50 flex items-center gap-2 text-[10px] font-black text-slate-300 uppercase italic">
                                    <Target size={14}/> Đây là giao diện xem thử nội dung đề thi
                                </div>
                            </div>
                        ))}
                    </div>
