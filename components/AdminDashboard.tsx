  ...
  const renderStudents = () => (
    <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm animate-fade-in">
        ...
        <div className="overflow-x-auto p-4">
            {/* Áp dụng tỷ lệ đề xuất: 25% - 15% - 15% - 30% - 15% */}
            <table className="w-full text-left text-[13px] border-collapse min-w-[1000px]">
                <thead className="bg-slate-50 text-slate-400 font-black uppercase border-b border-slate-100">
                    <tr>
                        ...
                        <th className="px-6 py-4 w-[30%] text-center">Thống kê học tập</th>
                ...
                <tbody className="divide-y divide-slate-50">
                        ...
                        return (
                            <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                                <td className="px-6 py-2.5">
                                    <div className="font-bold text-slate-800 leading-tight">{u.fullName}</div>
                                    <div className="text-[9px] font-black text-blue-500 uppercase mt-0.5">Khối {u.grade}</div>
                                </td>
                                <td className="px-6 py-2.5 font-mono text-blue-600 break-all">{u.username}</td>
                                <td className="px-6 py-2.5 font-mono text-slate-300 select-all break-all">{u.password}</td>
                                <td className="px-6 py-2.5">
                                    <div className="flex items-center justify-center gap-6">
                                        <div className="text-center">
                                            <div className="text-[8px] font-black text-slate-400 uppercase">Đã luyện</div>
                                            <div className="text-xs font-black text-slate-700">{sR.length} đề</div>
                                        </div>
                                        <div className="w-px h-6 bg-slate-200"></div>
                                        <div className="text-center">
                                            <div className="text-[8px] font-black text-slate-400 uppercase">Trung bình</div>
                                            <div className={`text-xs font-black ${avgScore >= 5 ? 'text-green-600' : 'text-slate-600'}`}>
                                ...
                                <td className="px-6 py-2.5 text-center">
                                    <button 
                                        ...
                                        className="inline-flex items-center justify-center p-2.5 bg-white text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all shadow-sm border border-slate-100 group-hover:border-red-100 group-hover:text-red-400"
                                        title="Xóa tài khoản"
                                    >
                                        <Trash2 size={18}/>
                                    </button>
                    ...
                    {filteredStudents.length === 0 && (
                        <tr><td colSpan={5} className="py-24 text-center text-slate-300 font-black uppercase text-xs tracking-widest bg-white">Không có tài khoản học sinh nào</td></tr>
                    )}
                </tbody>
            </table>
        </div>
