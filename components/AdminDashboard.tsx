...
import { 
    ...
    Clock, Eye, ListChecks, Check, Lightbulb, History, Download, FileText, Trash2
} from 'lucide-react';
                        ...
                        <div className="bg-white rounded-[3rem] w-full max-w-md flex flex-col overflow-hidden border-8 border-white shadow-2xl">
                            ...
                            <div className="p-10 space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Khối</label><select className="w-full p-4 bg-slate-50 border rounded-2xl p-4 text-xs font-black bg-slate-50" value={sForm.grade} onChange={e => setSForm({...sForm, grade: e.target.value as Grade})}><option value="12">Khối 12</option><option value="11">Khối 11</option><option value="10">Khối 10</option></select></div>
                    ...
                    <div className="fixed inset-0 bg-slate-900/90 z-[1000] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
                        <div className="bg-white rounded-[3.5rem] w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden border-8 border-white shadow-2xl">
                            ...
                            <div className="flex-1 overflow-y-auto p-10 bg-slate-50 space-y-8">
                                {(() => {
                                    const sResults = results.filter(r => r.studentId === selectedStudent.id || (selectedStudent.studentCode && r.studentCode === selectedStudent.studentCode.toUpperCase()));
                                ...
                                <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y">
                                            {results.filter(r => r.studentId === selectedStudent.id || (selectedStudent.studentCode && r.studentCode === selectedStudent.studentCode.toUpperCase())).sort((a,b)=>isAfter(parseISO(b.submittedAt), parseISO(a.submittedAt))?1:-1).map(r => (
                                                    ...
                                                    {q.type === 'group-tf' && q.subQuestions && (
                                                        <div className="space-y-3 pl-10">
                                                            {q.subQuestions.map((sq, si) => {
                                                                const sqUserAns = viewingResult.userAnswers?.[q.id]?.[si];
